...
- (void)test {
    ...
    for (NSUInteger i = 0; i < 10; i++) {
        if (flag) {
            ...
            flag = [manager createTableForObj:user primaryKeyStyle:(kDefault) completeHandler:^(NSString *tableName,PaintingliteSessionError * _Nonnull error, Boolean success) {
                        if (success) {
            ...
            if (flag) {
                user.name = @"CreaterOS";
                user.age = [NSNumber numberWithInt:21];
