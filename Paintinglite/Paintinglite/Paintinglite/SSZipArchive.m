...
/// 解压压缩包
/// @param path 压缩路径
/// @param destination 解压路径
/// @param preserveAttributes nil
/// @param overwrite 是否重写
/// @param password 密码
/// @param error 错误
/// @param delegate 协议
/// @param progressHandler 解压进度
/// @param completionHandler 完成 回调
+ (BOOL)unzipFileAtPath:(NSString *)path
          toDestination:(NSString *)destination
     preserveAttributes:(BOOL)preserveAttributes
              overwrite:(BOOL)overwrite
      ...
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    ...
    do {
        @autoreleasepool {
            ...
            ret = unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            //NSLog(@"unzGetCurrentFileInfo ret:%d",ret);
            if (ret != UNZ_OK) {
                success = NO;
                unzCloseCurrentFile(zip);
                break;
            ...
            //NSLog(@"preserveAttributes:%hhu, isDirectory:%hhu",preserveAttributes,isDirectory);
            ...
            if (isDirectory) {
                ...
                //创建压缩文件夹
                //NSLog(@"stringByDeletingLastPathComponent:%@",[fullPath stringByDeletingLastPathComponent]);
                [fileManager createDirectoryAtPath:[fullPath stringByDeletingLastPathComponent] withIntermediateDirectories:YES attributes:directoryAttr error:&err];
            }
            if (nil != err) {
                //NSLog(@"oops!!! create dict will be err...");
            ...
            //NSLog(@"fileIsSymbolicLink:%hhu",fileIsSymbolicLink);
            if (!fileIsSymbolicLink) {
                FILE *fp = fopen((const char*)[fullPath UTF8String], "wb");
                while (fp) {
                    int readBytes = unzReadCurrentFile(zip, buffer, 4096);
                    //NSLog(@"readBytes:%d,buffer:%s",readBytes,buffer);
                    if (readBytes > 0) {
